...
import { money, moneySF, moneyX as _moneyX, pct, fmtIsoD, fmtIsoDCompact } from "../../shared/lib/formatters";
...
import type { OppProject } from "../../shared/lib/constants";

// Ported from Component.renderVals() "Portfolio Insights" and "Business
// Development / Opportunity Tracker" sections (Ossa Fee Proposal App.dc.html
// lines 2470-2694 + 2900-2944). Ephemeral UI state (filters, sort, active
// inline-edit field, list/board toggle) lives as local component state here,
// matching how it lived in this.state.opp in the original but was never
// itself persisted to localStorage (only projects/year/pendingTarget are).

const OPP_STATUSES = ["New Lead", "Write Proposal", "Pending Approval", "Won / In Process", "Completed", "Lost", "Cancelled"];

export function usePipeline() {
  const { state, updStore, setOppState, addOppProject, updateOppProject, openOrCreateOppProject, removeOppProject, addOppSplit, removeOppSplit, updateOppSplit, setOppPendingTarget } =
    useAppState();
  const { store, opp } = state;
  ...
  // ---- Portfolio Insights (cross-project) ----
  const portfolioRows = store.order.map((id) => {
    ...
    verdict = goWT === 0 ? null : goPct >= 0.75 ? "GO" : goPct >= 0.6 ? "CAUTION" : "NO-GO";
  ...
  opp.projects.forEach((p) => (outcomeCounts[p.status] = (outcomeCounts[p.status] || 0) + 1));
  ...
  const oppYearsSet = new Set(opp.projects.map((p) => oppEffectiveYear(p)));
  ...
  const oppAllYear = opp.projects.filter((p) => oppEffectiveYear(p) === oppYear);

  const oppSumWhere = (pred: (p: OppProject) => boolean, field: keyof OppProject) => oppAllYear.filter(pred).reduce((a, p) => a + (Number(p[field]) || 0), 0);
  const oppComputedExpected = (p: OppProject) => ((Number(p.potentialFee) || 0) * (Number(p.chances) || 0)) / 100;
  ...
  const oppTotalInvoiced = opp.projects.reduce((a, p) => a + oppInvoicedInYear(p, oppYear), 0);
  ...
  const oppTarget = opp.pendingTarget || 500000;
  ...
  const oppColDefs: { key: string; label: string; num?: boolean; w: string }[] = [
    ...
    { key: "client", label: "Client", w: "13%" }, { key: "project", label: "Project", w: "14%" },
  ...
  let oppFiltered = oppAllYear.filter((p) => {
    ...
    if (q && !((p.client || "").toLowerCase().includes(q) || (p.project || "").toLowerCase().includes(q))) return false;
    return true;
  });
  if (sortKey) {
    const key = sortKey as keyof OppProject;
    const dir = sortDir;
  ...
  const moneyDisplay = (p: OppProject, field: "potentialFee" | "invoiced" | "remaining") =>
    activeField === p._id + "::" + field ? String(p[field] ?? 0) : oppFmtMoney(p[field] as number);
  ...
  const oppRows = oppFiltered.map((p) => ({
    ...
    onDateChange: (v: string) => updateOppProject(p._id, "date", v),
    onStatusChange: (v: string) => updateOppProject(p._id, "status", v),
    showLostReason: p.status === "Lost",
    lostReason: p.lostReason || "",
    onLostReasonChange: (v: string) => updateOppProject(p._id, "lostReason", v),
    onClientChange: (v: string) => updateOppProject(p._id, "client", v),
    onProjectChange: (v: string) => updateOppProject(p._id, "project", v),
    yearSplits: p.yearSplits || [],
    splitOpen: splitOpenId === p._id,
    onToggleSplit: () => setSplitOpenId(splitOpenId === p._id ? null : p._id),
    splitRows: (p.yearSplits || []).map((s, i) => ({
      year: s.year,
      onYearChange: (v: string) => updateOppSplit(p._id, i, "year", v),
      amountDisplay: activeField === p._id + "::split" + i ? String(s.invoiced ?? 0) : oppFmtMoney(Number(s.invoiced) || 0),
      onAmountFocus: () => setActiveField(p._id + "::split" + i),
      onAmountBlur: () => setActiveField(null),
      onAmountChange: (v: string) => updateOppSplit(p._id, i, "invoiced", oppParseMoney(v)),
      onRemove: () => removeOppSplit(p._id, i),
    })),
    onAddSplit: () => addOppSplit(p._id),
    onPotentialFeeFocus: () => setActiveField(p._id + "::potentialFee"),
    onInvoicedFocus: () => setActiveField(p._id + "::invoiced"),
    onRemainingFocus: () => setActiveField(p._id + "::remaining"),
    onFieldBlur: () => setActiveField(null),
    onPotentialFeeChange: (v: string) => updateOppProject(p._id, "potentialFee", oppParseMoney(v)),
    onInvoicedChange: (v: string) => updateOppProject(p._id, "invoiced", oppParseMoney(v)),
    onRemainingChange: (v: string) => updateOppProject(p._id, "remaining", oppParseMoney(v)),
    onChancesChange: (v: number) => updateOppProject(p._id, "chances", v),
    hasProject: p.status !== "New Lead",
    onOpenProject: () => openOrCreateOppProject(p._id),
    onRemove: () => removeOppProject(p._id),
  }));
  ...
  const onOppDropCol = (status: string) => (e: DragEvent) => {
    ...
    if (id) updateOppProject(id, "status", status);
  };
  const oppBoardColumns = OPP_STATUSES.map((status) => {
    ...
    return {
      ...
      cards: rows.map((p) => ({
        id: p._id,
        onDragStart: onOppDragStart(p._id),
        project: p.project || "Untitled",
        client: p.client || "—",
  ...
  return {
    ...
    oppYearOptions,
    onOppYearChange: (v: string) => setOppState({ year: v }),
    onOppAddProject: addOppProject,
    ...
    onOppTargetChange: (v: string) => setOppPendingTarget(oppParseMoney(v)),
    oppProgressPct,
    ...
    oppBoardColumns,
    onOpenProjectFromRow: (id: string) => {
      if (!store.projects[id]) return;
      updStore((st) => { st.currentId = id; });
    },
  };
}
